...
import { replaceState } from "../urlState/index.js"
...
const callLinkToLeaveRoute = ({routes, last_route}) => {
    routes.forEach( (route, id) => {
        var value = history.state?.value
        value = value !== undefined ? value : "/"
        var last_rout = last_route === undefined ? value : last_route
        if(route.path == last_rout){
           document.querySelectorAll("."+config_link_to_click_leave_one_route+id)?.forEach(e => e.click())
        }
    })
}
...
const callLinkToInRoute = ({routes, actual_route}) => {
    routes.forEach( (route, id) => {
        var value = history.state?.value
        value = value !== undefined ? value : "/"
        var actual_rout = actual_route === undefined ? value : actual_route
        if(route.path == actual_rout){
           document.querySelectorAll("."+config_link_to_click_enter_one_route+id)?.forEach(e => e.click())
        }
    })
}
